...
public class HuffProcessor {
	...
	private int[] getCounts(BitInputStream in){
		int[] output = new int[ALPH_SIZE+1];
		while (true){
	...
	private HuffNode makeTree(int[] freq){
		...
		while (pq.size() > 1) {
			...
			HuffNode t = new HuffNode(0, left.weight+right.weight,  left, right); //value??
	...
	public void decompress(BitInputStream in, BitOutputStream out){
		...
		while (true){
			...
			if (bits == -1) {
				...
				if (current.left == null && current.right == null) { //current is a leaf node
					if (current.value == PSEUDO_EOF)
						break;   // out of loop
					else {
						out.writeBits(BITS_PER_WORD, current.value); //write BITS_PER_WORD bits to output for current.value;
